package org.jeecg.modules.zxecg.config.shiro;
...
@Component
@Slf4j
public class ZxecgSysShiroRealm extends AuthorizingRealm {
	@Lazy
    @Resource
    ...
    public SysUserVO checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        SysUserVO loginUser = getLoginUser(username, sysUserService, redisUtil);
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        String cacheToken = String.valueOf(redisUtil.get(CommonConstant.PREFIX_ZXECG_USER_TOKEN + token));
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                String newAuthorization = JwtUtil.sign(userName, passWord);
                // 设置超时时间
                redisUtil.set(CommonConstant.PREFIX_ZXECG_USER_TOKEN + token, newAuthorization);
                redisUtil.expire(CommonConstant.PREFIX_ZXECG_USER_TOKEN + token, JwtUtil.EXPIRE_TIME *2 / 1000);
...
//			}
     ...
     * @return
     */
    public static SysUserVO getLoginUser(String username, ISysUserService sysUserService, RedisUtil redisUtil) {
        SysUserVO loginUser = null;
        String loginUserKey = ZxecgCacheConstant.ZXECG_SYS_USER_CACHE + "::" + username;
        //【重要】此处通过redis原生获取缓存用户，是为了解决微服务下system服务挂了，其他服务互调不通问题---
//        redisUtil.del(loginUserKey);
        if (redisUtil.hasKey(loginUserKey)) {
            try {
                loginUser = (SysUserVO) redisUtil.get(loginUserKey);
                //解密用户
            ...
            // 查询用户信息
            loginUser = sysUserService.getSystemUserVOByUserId(Long.valueOf(username));
        }
        return loginUser;
    }
