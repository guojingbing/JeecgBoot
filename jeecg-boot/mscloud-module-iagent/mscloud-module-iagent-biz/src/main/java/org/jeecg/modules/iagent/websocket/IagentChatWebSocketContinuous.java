...
@Component
@Slf4j
@ServerEndpoint("/iagentWebsocket/c/{userId}")
public class IagentChatWebSocketContinuous {
    /**线程安全Map*/
     ...
     * Redis触发监听名字
     */
    public static final String REDIS_TOPIC_NAME = "iagentSocketContinuousHandler";
    ...
    private void setJeecgRedisClient(JeecgRedisClient jeecgRedisClient){
        IagentChatWebSocketContinuous.jeecgRedisClient = jeecgRedisClient;
    }
    ...
    public void onMessage(String message, @PathParam(value = "userId") String userId) {
        if(!"ping".equals(message) && !WebsocketConst.CMD_CHECK.equals(message)){
            ...
            if(StringUtils.isNotBlank(messageJson.getString("source"))&&messageJson.getString("source").equalsIgnoreCase("chatAudio")){
                ...
                byte[] audioStream=content!=null?Base64.decode(content):null;
...
//                    log.error("从websocket缓存中获取IAliyunSpeechRecognizer");
                    SpeechReqProtocol.State state=(SpeechReqProtocol.State)recognizer.getTranscriberState();
                    if(state==SpeechReqProtocol.State.STATE_CLOSED||state==SpeechReqProtocol.State.STATE_FAIL){
                        recognizer= InstanceBeanUtils.getBean(AliyunSpeechRecognizerImpl.class);
                        isBegin=true;
                        recognizer.init();
                        recognizerPool.put(userId,recognizer);
                ...
                recognizer.executeRecognizeTranscriber(audioStream, null, isBegin, isEnd, new SpeechTranscriberListener() {
                    //识别出中间结果。仅当setEnableIntermediateResult为true时，才会返回该消息。
                    @Override
                    public void onTranscriptionResultChange(SpeechTranscriberResponse response) {
//                        System.out.println("onTranscriptionResultChange>>>task_id: " + response.getTaskId() +
...
//                        System.out.println("onTranscriberStart>>>task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
                    }

                    @Override
                    public void onSentenceBegin(SpeechTranscriberResponse response) {
//                        System.out.println("onSentenceBegin>>>task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
...
//                                        }
                    ...
                    public void onTranscriptionComplete(SpeechTranscriberResponse response) {
                        System.out.println("onTranscriptionComplete>>>task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
                    }

                    @Override
                    public void onFail(SpeechTranscriberResponse response) {
                        //task_id是调用方和服务端通信的唯一标识，遇到问题时，需要提供此task_id。
                        System.out.println("onFail>>>task_id: " + response.getTaskId() +  ", status: " + response.getStatus() + ", status_text: " + response.getStatusText());
                    }
                });
            }
        }else{
...
//                    System.out.println("name: " + response.getName() + ", status: " + response.getStatus());
    ...
    public void sendMessage(String message) {
        ...
        jeecgRedisClient.sendMessage(IagentChatWebSocketContinuous.REDIS_TOPIC_NAME, baseMap);
    }
