  ...
  <!-- 子表单区域 -->
    <a-tabs v-model:activeKey="activeKey" animated @change="handleChangeTabs">
      <a-tab-pane tab="运单货物表" key="lttlWaybillGoods" :forceRender="true">
        <JVxeTable
          v-if="lttlWaybillGoodsTable.show"      
          keep-source
          resizable
...
<script lang="ts" setup>
    ...
    //渲染流程表单数据
    const queryByIdUrl = '/lttl/shipping/po/info';
    async function initFormData(){
      if(props.formBpm === true){
        ...
        requestSubTableData(queryLttlWaybillGoods, {id: data.id}, lttlWaybillGoodsTable, ()=>{
          lttlWaybillGoodsTable.show = true;
        });
        requestSubTableData(queryLttlWaybillFee, {id: data.id}, lttlWaybillFeeTable, ()=>{
          lttlWaybillFeeTable.show = true;
        });
        // 隐藏底部时禁用整个表单
