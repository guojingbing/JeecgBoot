<template>
  ...
  <BasicTable @register="registerTable" :rowSelection="rowSelection">
    <!--插槽:table标题-->
    <template #tableTitle>
      <a-button type="primary" preIcon="ant-design:plus-outlined" @click="selectAddUser">选择用户</a-button>
...
<script lang="ts" setup>
  ...
  import { departUserList, linkDepartUserBatch, unlinkDepartUserBatch } from '../../departUser/depart.user.api';
  import { departAdminUserList, changeDepartChargePerson, changeDepartChargePersonBatch } from '../depart.api';
  import { userInfoColumns, userInfoSearchFormSchema } from '../depart.data';
  ...
  async function unlinkDepartUser(idList, confirm) {
    if (!departId.value) {
      ...
      setLoading(true);
      try {
        await changeDepartChargePersonBatch({ userIds: unref(idList), departId: departId.value, status: false }, confirm);
        return reload();
      } finally {
        setLoading(false);
      }
  ...
  async function onSelectUserOk(options, userIdList) {
    if (userIdList.length > 0) {
      try {
        ...
        await changeDepartChargePerson({ userIds: userIdList, departId: departId.value, status: true });
        reload();
      } finally {
        setLoading(false);
      }
  ...
  function getTableAction(record): ActionItem[] {
    ...
    return [{ label: '取消关联', onClick: unlinkDepartUser.bind(null, [record.id], false) }];
  }
